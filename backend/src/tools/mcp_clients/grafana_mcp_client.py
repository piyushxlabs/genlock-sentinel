...
class GrafanaMCPClient:
    """Async client managing queries to Grafana MCP and Tempo MCP."""
    ...
    async def query_loki_logs(
        ...
        """Executes Loki log search with exponential backoff and silence-over-guessing fallback."""
        if simulate_timeout:
                ...
                try:
                    async with httpx.AsyncClient(timeout=5.0) as client:
                        ...
                        if resp.status_code == 200:
                            ...
                            return QueryLokiLogsOutput(success=True, result=lines, error=None)
                except Exception:
        ...
        return QueryLokiLogsOutput(
            success=True,
            result=MOCK_LOKI_RESPONSES["render-07"],
            error=None,
        )
    ...
    async def find_slow_requests(
        ...
        """Invokes Grafana Sift investigation on Tempo spans with exponential backoff."""
        if simulate_timeout:
        ...
        return FindSlowRequestsOutput(success=True, result=result_data, error=None)
    ...
    async def get_trace_by_id(
        ...
        """Retrieves full trace by frame_id from Tempo MCP with exponential backoff."""
        if simulate_timeout:
        ...
        return GetTraceByIdOutput(success=True, result=trace_data, error=None)
